...
export class Market {
  ...
  async getImpliedAPY(lastLnImpliedRate: uint256, daysLeft: uint256) {
    ...
    let impliedAPY = expValue.minus(1).multipliedBy(new BigNumber("1e18"));

    const log = new BigNumber(Math.log10(expValue.toNumber()));

    const exponent = new BigNumber(daysLeft).times(log).dividedBy(365);

    const PtToAsset = new BigNumber(
      new BigNumber("1").dividedBy(Math.pow(10, exponent.toNumber()))
